...
- **D-04:** Pacing: original inter-event gaps ÷ speed. Default speed=1 (script arg override). **speed>1 is a headless-testing configuration only, never user-facing** (a 5s window spans 100s of match time at 20×). ~~Pre-match segment plays at full original pacing (no compression, no skip).~~
  - **AMENDED 2026-07-18 (quick task `260718-3cm`) — pre-match dead time IS clamped.** Inter-event sleeps are capped: `min(originalGap / speed, maxGapMs)`, clamp applied **after** speed division. Production default `maxGapMs = 5000ms`, env-overridable via `REPLAY_MAX_GAP_MS`.
  - **Why the original wording was wrong:** it assumed pre-match gaps were tolerable. Real historical data disproves this — reference repo `txodds-latency-test-app`, fixture `18187298`: the fixture's history spans ~5 days and contains a single **430,164,128 ms (4.98-day)** gap between events #2 and #3 (the dead zone between early `coverage_update`/`comment` records and the ~1h-before-kickoff `connected` record). Uncompressed, the replay emits 2 events then sleeps ~5 days, which makes the **D-06/D-08 demo choreography physically impossible**. `speed` alone does not rescue it (still ~7.2 min of dead air at 20×), and D-03 rebasing does not help — it shifts all timestamps by a constant delta but **preserves inter-event gaps**.
  - **Scope of the change:** match-time pacing is unaffected in practice (in-play gaps sit far below the 5s ceiling), so the "no distortion of match pacing" intent of D-04 is preserved. The emitter's own default remains `Infinity` (no clamp) — the clamp is supplied by the production caller (`ReplayStarterService`), which the emitter module doc explicitly sanctions.
