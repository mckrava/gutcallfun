...
- [x] **Phase 02.1: Placeholder API Surface & Fake Realtime Contract** *(INSERTED)* - Full REST + socket.io contract with realistic mock payloads so the UI developer can integrate immediately, ahead of Phases 3-5 (completed 2026-07-18)
...
- [x] 01-04-PLAN.md — Schema push, empty-diff gate, and boot verification (DATA-01, DATA-03, DATA-04)
...
- [x] 02-05-PLAN.md — SSE live ingest: ported upstream.ts + StreamManagerService (resume, watchdog, backoff, AbortController shutdown) feeding the shared pipeline (INGST-02, INGST-01, RPLY-02)
- [x] 02-06-PLAN.md — Replay emitter: ported replay.ts (rebasing, uncompressed pre-match pacing), Source A/B loader, is_replay-guarded auto-wipe starter, replay:create dev script (RPLY-01, RPLY-02, RPLY-03)
...
- [x] 02.1-01-PLAN.md — Packages (behind a legitimacy checkpoint), WEB_APP_ORIGIN fail-fast config, and bootstrap wiring: ValidationPipe, CORS, Swagger (API-01, API-02, WS-03)
...
- [x] 02.1-04-PLAN.md — socket.io gateway, fake game cycle with guaranteed timer teardown, snapshot match clock, WS-CONTRACT.md (WS-01, WS-02, WS-03)
...
| 02.1. Placeholder API Surface & Fake Realtime Contract *(INSERTED)* | 5/5 | Complete    | 2026-07-18 |
