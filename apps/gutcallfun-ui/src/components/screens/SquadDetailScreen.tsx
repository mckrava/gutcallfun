...
// Ported verbatim from the "Squad detail" screen in the old template (isSquadDetail block).
  ...
  return (
    ...
    >
      ...
      <div style={{ marginTop: 10, display: "flex", alignItems: "center", gap: 13 }}>
        <div style={{ width: 52, height: 52, borderRadius: 15, background: "#0D1626", boxShadow: `0 0 0 2px ${vm.openSquad.ring}`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 26, flexShrink: 0 }}>{vm.openSquad.emoji}</div>
        <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 3 }}>
          <div style={{ fontFamily: "'Barlow Condensed',sans-serif", fontStyle: "italic", fontWeight: 700, fontSize: 26, letterSpacing: ".4px" }}>{vm.openSquad.name}</div>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}><span style={{ fontSize: "11.5px", color: "rgba(220,230,245,.45)" }}>{vm.openSquad.count}</span><span style={{ fontSize: 10, fontWeight: 700, letterSpacing: 1, color: "#FFD84D", background: "rgba(255,216,77,.12)", borderRadius: 6, padding: "2px 7px", fontFamily: "ui-monospace,Menlo,monospace" }}>#{vm.openSquad.code}</span></div>
        </div>
