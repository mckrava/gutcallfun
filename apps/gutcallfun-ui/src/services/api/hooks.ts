...
export function useGameEvents(gameId: number | null, query?: PaginationQuery) {
  return useQuery({
...
export function useGameQuestions(gameId: number | null, query?: PaginationQuery) {
  return useQuery({
...
export function useUpdateUser(userId: string) {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (body: UpdateUserBody) => usersApi.update(userId, body),
    onSuccess: () => qc.invalidateQueries({ queryKey: queryKeys.users.detail(userId) }),
  });
}
...
// ---- Squads ----
export function useSquads(query?: PaginationQuery) {
